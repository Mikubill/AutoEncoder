...
meson build --prefix=/usr/local -Ddefault_library=static
cd build
ninja
ninja install
...
./autogen.sh
./configure --cache-file=/tmp/configure.cache --prefix=/usr/local --install=/usr/local/lib/vapoursynth 
make -j$(nproc)
make install 
...
cd fftw-3.3.10
./configure --cache-file=/tmp/configure.cache --prefix=/usr/local --enable-float --enable-threads --disable-shared
make -j$(nproc)
make install
...
./waf configure --prefix=/usr/local --libdir=/usr/local/lib/ --install=/usr/local/lib/vapoursynth 
./waf build
./waf install
...
# build vs-plugin: vs-tcanny
cd /opt/vs
git clone --depth 1 https://github.com/HomeOfVapourSynthEvolution/VapourSynth-TCanny    
